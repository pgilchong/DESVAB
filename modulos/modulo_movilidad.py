    ...
    def __init__(
        ...
        def cargar_datos(
            ...
            with warnings.catch_warnings(record=True):
                ...
                df_cuad = excel.parse(
                    sheet_name, usecols=['num_cuadrante', 'CO2_real', 'FE_num_pub', 'FE_num_priv'],
                    index_col='num_cuadrante'
                    )
            df_cuad['CO2_real'] = df_cuad['CO2_real'] / (10**6)  # Convertir a toneladas
            df_cuad['FE'] = df_cuad['FE_num_pub'] + df_cuad['FE_num_priv'] # Sumar kWh de públicos y privados
            df_cuad['FE'] = df_cuad['FE'] / (10**3)  # Convertir a MWh
            df_cuad = df_cuad.drop(columns=['FE_num_pub', 'FE_num_priv'])
            cols = ['CO2_real', 'FE']
            ...
            neighborhoods_with_data = gdf_barrios[(gdf_barrios['CO2_real'] > 0)]
            for neighborhood, fila in gdf_barrios.iterrows():
            ...
            for cuad, fila in gdf_cuad.iterrows():
                ...
                if (fila['CO2_real'] == 0) or (isclose(matriz_overlap.loc[cuad].sum(), 1, abs_tol=1e-5)):
                    continue
        ...
        self.vector_m1, self.vector_m2, self.vector_m3 = {}, {}, {}
        for vector in [self.vector_m1, self.vector_m2, self.vector_m3]:
